define(function(require) {
	...
	var app = {
		...
		devicesRenderDevice: function(args) {
			...
			if (data.extra.hasOwnProperty('provision') && data.extra.provision.hasOwnProperty('settings') && data.extra.provision.settings.hasOwnProperty('keys')) {
				_.each(data.extra.provision.settings.keys, function(value) {
					...
					_.each(value.data, function(val, key) {
						if (val) {
							...
							templateDevice
								...
								.addClass('active');
                                                        templateDevice.find(valueSelector + ' [name="provision.settings.keys.' + value.id + '[' + key + '].key.line"]')
								.val(_.get(val, 'value.line'));
							templateDevice.find(valueSelector + ' [name="provision.settings.keys.' + value.id + '[' + key + '].key.value"]')
								.val(_.get(val, 'value.value'));
							templateDevice.find(valueSelector + ' [name="provision.settings.keys.' + value.id + '[' + key + '].key.label"]')
				...
				templateDevice.find('.keys').each(function() {
					...
					$this.sortable({
						...
						stop: function(e, ui) {
							...
							if ($this.data('section') === 'linesKeys') {
								$this
									.find('.control-group')
										.first()
											.addClass('warning')
			...
			if ('call_restriction' in formData) {
				_.each(formData.call_restriction, function(restriction, key) {
					if (key in originalData.extra.restrictions && originalData.extra.restrictions[key].disabled) {
                        ...
                        if (_.has(formData, 'provision.settings.keys')) {
				/**
				 ...
				 */
                                 ...
                                 _.each(formData.provision.settings.keys, function(value, key, list) {
                                  
                                  var keys = {};
                                  originalData.provision.settings = {};
                                  originalData.provision.settings.keys = {};
					...
					if (_.isEmpty(keys)) {
                                        ...
                                        delete originalData.provision.settings[key] ;  
					} else {
				...
				delete formData.provision.settings.keys;
